...
def depth_to_frequency(depth_norm):
    ...
    return 2500 - (depth_norm * 1800)  # 2500Hz (close) to 700Hz (far)
...
def export_frame_audio(frame_idx, azimuths_deg, duration_s=2.0, sr=48000, out_dir=AUDIO_DIR, depth_npy_dir=DEPTH_NPY_DIR):
    ...
    if len(azimuths_deg) == 0:
        print(f"  ⚠ No azimuths detected for frame {frame_idx}, using sweep across field")
    ...
    for az in azimuths_deg:
        ...
        dist_norm = float(col_mean[col])  # 0..1 (far)

        # Generate filtered noise with frequency based on depth
        center_freq = depth_to_frequency(dist_norm)
        bandwidth = 300 + (dist_norm * 200)  # Wider bandwidth for farther objects
...
def play_audio_file(audio_path):
    ...
    if not AUDIO_PLAYBACK_AVAILABLE:
        print("  ⚠ pygame not available, skipping playback")
        return False
...
def main():
    ...
    if args.run_midas:
        ...
        print("\n[2/4] ⏭️  Skipping MiDaS (use --run-midas to enable)")

    # 3) Detect open-space candidates
    print("\n[3/4] 🗺️  Detecting open-space candidates...")
    ...
    if args.live:
        if not AUDIO_PLAYBACK_AVAILABLE:
            print("\n⚠ Audio playback not available. Install with:")
            ...
            if args.continuous:
                ...
                print("\n▶️  Playing all frames continuously...")
        ...
        print("\n💡 Use --live flag to enable live audio playback")

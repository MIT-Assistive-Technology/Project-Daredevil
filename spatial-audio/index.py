...
try:
    ...
    if not PYOAL_AVAILABLE:
        warnings.warn("OpenAL library couldn't be found. Install with: brew install openal-soft")
...
class SpatialAudioEngine:
    ...
    def _initialize_openal(self):
        ...
        try:
            ...
            print("Make sure audio device is available and not in use by another application")
    ...
    def _update_listener(self):
        ...
        openal.al.alListener3f(openal.AL_POSITION, float(self.listener_position[0]), float(self.listener_position[1]), float(self.listener_position[2]))
        
        # Set listener orientation (forward vector, then up vector)
        orientation_array = np.concatenate([self.listener_forward, self.listener_up]).astype(np.float32)
    ...
    def _generate_white_noise(self, duration: float = 1.0, sample_rate: int = 44100) -> np.ndarray:
        """
            ...
            try:
                with self.lock:
                    ...
                    for object_id, obj_data in self.objects.items():
                        ...
                        # Update volume
                        openal.al.alSourcef(source_id, openal.AL_GAIN, obj_data['volume'])
                        
                        # Update playback state
                        state = openal.al.alGetSourcei(source_id, openal.AL_SOURCE_STATE)
    ...
    def start(self):
        ...
        self.audio_thread = threading.Thread(target=self._update_audio_loop, daemon=True)
...
if __name__ == "__main__":
    ...
    try:
        ...
        while True:
            ...
            print(f"Active objects: {stats['total_objects']}, Active sources: {stats['active_sources']}")

...
class EnhancedDepthProcessorWithPersistence(EnhancedDepthProcessor):
    ...
    def __init__(
        ...
        logger.info(f"Persistence duration: {self.persistence_config.persistence_duration}s")
    ...
    def get_enhanced_depth_for_spatial_audio(
        ...
        result = super().get_enhanced_depth_for_spatial_audio(frame, bbox, object_id, object_class)
    ...
    def _update_object_state(
        ...
        if object_id not in self.tracked_objects:
            ...
            # Update histories
            obj_state.confidence_history.append(depth_result.get('quality_metrics', {}).overall_confidence)
    ...
    def _get_persistence_info(self, object_id: str) -> Dict[str, Any]:
        ...
        predicted_position = self._predict_object_position(obj_state) if self.persistence_config.motion_prediction_enabled else None
    ...
    def _get_depth_terminology(self, raw_depth: float) -> Dict[str, Any]:
        ...
        terminology = {
            ...
            'background_relationship': self._classify_background_relationship(raw_depth),
    ...
    def _predict_object_position(self, obj_state: ObjectState) -> Optional[Dict[str, Any]]:
        ...
        motion_vector = (current_center[0] - previous_center[0], current_center[1] - previous_center[1])
        
        # Predict next position (simple linear prediction)
        predicted_center = (current_center[0] + motion_vector[0], current_center[1] + motion_vector[1])
        ...
        return {
            ...
            'prediction_confidence': min(1.0, len(recent_bboxes) / 5.0)  # More history = higher confidence
        }
    ...
    def update_missing_objects(self):
        ...
        for object_id, obj_state in self.tracked_objects.items():
            ...
            should_remove = (
                time_since_last_seen > self.persistence_config.persistence_duration or
                obj_state.missing_frames > self.persistence_config.max_missing_frames
            )
    ...
    def _apply_persistence_effects(self, obj_state: ObjectState):
        ...
        if len(obj_state.confidence_history) > 0:
            ...
            decayed_confidence = max(0.0, current_confidence - self.persistence_config.confidence_decay_rate)
        ...
        if len(obj_state.bbox_history) > 0:
            ...
            expansion = int(self.persistence_config.bbox_expansion_rate * obj_state.missing_frames)
        ...
        if self.persistence_config.depth_prediction_enabled and len(obj_state.depth_history) > 0:
    ...
    def get_persistent_objects(self) -> List[Dict[str, Any]]:
        ...
        for object_id, obj_state in self.tracked_objects.items():
            ...
            if not obj_state.is_active and obj_state.missing_frames > self.persistence_config.max_missing_frames:
                continue
            
            # Get current or predicted bounding box
            if obj_state.is_active:
                current_bbox = obj_state.bbox
                confidence = obj_state.depth_info.get('quality_metrics', {}).overall_confidence
            else:
    ...
    def set_persistence_config(self, config: PersistenceConfig):
        ...
        logger.info(f"Persistence config updated: enabled={config.enabled}, duration={config.persistence_duration}s")
...
def create_enhanced_depth_processor_with_persistence(
    ...
    return EnhancedDepthProcessorWithPersistence(model_id=model_id, persistence_config=persistence_config)
